...
void Raft::AppendEntries1(const raftRpcProctoc::AppendEntriesArgs *args, raftRpcProctoc::AppendEntriesReply *reply)
{
    ...
    *     如果Node B的任期号与Node A相同，但Node A发现Node B的日志至少与自己的日志一样新（即具有更大的日志索引或相等的索引但具有更大的任期号），
    *     Node A应该承认Node B是Leader，并转换回Follower状态。
    ...
    */
    ...
    // TODO：这里目前当args.PrevLogIndex == rf.lastSnapshotIncludeIndex与不等的时候要分开考虑，可以看看能不能优化这块
    ...
    else    // matchLog(args->prevlogindex(), args->prevlogterm()) == false
    {
                           ...
                           raftRpcProctoc::InstallSnapshotResponse *reply)
{
    ...
    // 截断日志，修改commitIndex和lastApplied [截断日志用以快照替换]
...
void Raft::leaderHearBeatTicker()
{
    while (true)
    {
        // TODO 不是leader的话就没有必要进行后续操作，况且还要拿锁，很影响性能，目前是睡眠，后面再优化优化
        while (m_status != Leader)
        {
...
void Raft::Snapshot(int index, std::string snapshot)
{
    ...
    // TODO 这里会不会使得将为提交的 raftstate 数据写入了文件
