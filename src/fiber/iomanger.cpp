...
namespace monsoon
{
     ...
     * @details 初始化管道pipe，并注册管读端描述符(边缘触发)
     */
    ...
    int IOManager::addEvent(int fd, Event event, std::function<void()> cb)
    {
        ...
        {
            lock.unlock();
            RWMutex::WriteLock lock2(mutex_);   // 依靠析构释放
        ...
        epoll_event epevent;
        epevent.events = EPOLLET | fd_ctx->events | event;  //TODO: fd 加入的时候是否设置了 非阻塞
    ...
    {   
        ...
        FdContext *fd_ctx = fdContexts_[fd];    // fd上无事件的时候，指针也不为空，只有析构时才释放
        lock.unlock();
        ...
        while (true)
        {
            ...
            {   
                ...
                else
                {
                    if(ret == 0) 
                    {
                        std::cout << "+++++++++++++++++ non event ===========" << std::endl;
                    }
                    break;
                }
            } while (true);

            // TODO 这里就收集定时器，啥时候添加的啊
            /* 收集所有超时定时器，执行回调函数 */
