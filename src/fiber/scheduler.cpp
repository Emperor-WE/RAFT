...
namespace monsoon
{
     ...
     *  加上前⾯协程模块的cur_fiber和cur_thread_fiber，每个线程总共可以记录三个协程的上下⽂信息
     */
    ...
    Scheduler::Scheduler(size_t threads, bool use_caller, const std::string &name)
    {
        ...
        /**
         * @details use_caller: 是否将当前线程也作为 调度线程
         *      1.当主线程（调度器线程）不参与调度时，即use_caller为false时，就必须要创建其他线程进⾏协程调度。
         *      2.当主线程也参与调度时，即use_caller为true
        ...
        if(use_caller)
        {
            ...
            // 总线程数 -1【不包含caller线程，因为caller线程不受调度器调度】
            --threads;
            ...
            /* 设置当前线程的调度器线程（caller thread）*/
            cur_scheduler = this;

            /* 初始化当前线程的调度协程(该线程不会被调度器调度)，调度结束后，返回主协程. 调度协程不被调度器调度 */
